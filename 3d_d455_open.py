    ...
    def read_color_depth(self):
        try:
            ...
            if(not color_frame is None):
                addd = color_frame.get_data()
                color_image = np.asanyarray(color_frame.get_data())
                print(addd)
            if(not depth_frame is None):
                addd = depth_frame.get_data()
                depth_image = np.asanyarray(depth_frame.get_data())
                #depth_image = depth_image[:, :, [2, 1, 0]]
                # color_image = depth_image[:, :, [2, 1, 0]]
                #print(addd)
...
if __name__ == "__main__":
    ...
    if(cam.isOpened()):
        while True:
            ret, frame = cam.read();
            if(ret):
                #frame[0].as_motion_frame().get_motion_data()
        ...
        while True:
            ...
            if(ret):
                ...
                if(not color is None):
                    ...
                    distance = depth[point_x, point_y]
                    cv2.circle(color, (100,250), 8, (0,0,255), -1)
                    cv2.putText(color, "{} mm".format(distance), (point_x, point_y-10),0,1,(0,0,255),2)
            ...
            # gyro_data = np.asarray([gyro.x, gyro.y, gyro.z])
